//! Scenario: drive the full Underdark pressure → raid → surface loop.
//!
//! 1. Start a fresh headless server with `--history-warp-ticks 0`.
//! 2. Clear battle history so we only see post-start records.
//! 3. Call `dm/force_underdark_pressure` to skip ~10 slow ticks of accumulation.
//! 4. Poll `dm/underdark_pressure` until a raid has spawned (score reset + tick set).
//! 5. Query `WarPartyMember` entities tagged with the `underdark` `FactionBadge`.
...
//!
//! ```bash
//! cargo run -p ralph -- --scenario underdark_e2e
//! ```
//!
...
impl Scenario for UnderdarkE2e {
    fn name(&self) -> &str {
        "underdark_e2e"
    }
    ...
    fn run(&self) -> Result<()> {
        ...
        let (initial_score, initial_tick) = server.dm_underdark_pressure()?;
        tracing::info!(initial_score, initial_tick, "Initial underdark pressure");
        server.dm_force_underdark_pressure()?;
        tracing::info!("dm/force_underdark_pressure → score=1.0");

        // ── 4. Wait for the raid spawn by observing pressure reset ────────────
        tracing::info!("Polling dm/underdark_pressure until last_raid_tick advances …");
        let deadline = Instant::now() + SPAWN_TIMEOUT;
        loop {
            let (score, last_raid_tick) = server.dm_underdark_pressure()?;
        ...
        // ── 5. Find the Underdark raid party members ──────────────────────────
        tracing::info!("Polling for WarPartyMember with FactionBadge=underdark …");
        let deadline = Instant::now() + SPAWN_TIMEOUT;
        let raid_entities: Vec<u64> = loop {
            ...
            if !matching.is_empty() {
                tracing::info!(count = matching.len(), "Underdark raid party present");
                break matching;
            }
            if Instant::now() > deadline {
                bail!(
                    "No Underdark faction WarPartyMember entities within {:?}",
                    SPAWN_TIMEOUT
                );
            }
            sleep(POLL);
        ...
        // ── 7. Best-effort: wait for a BattleHistory record mentioning underdark
        ...
        tracing::info!(
            "Best-effort polling dm/battle_history for an underdark record …"
        );
        ...
        while Instant::now() < deadline {
            ...
            if records.iter().any(|r| {
                r["winner_faction"].as_str() == Some("underdark")
                    || r["loser_faction"].as_str() == Some("underdark")
            }) {
                saw_record = true;
                tracing::info!("BattleHistory contains an underdark record");
                break;
            }
            sleep(POLL);
        }
        if !saw_record {
            tracing::warn!(
                "No underdark BattleHistory record observed within {:?} — \
