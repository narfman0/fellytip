...
| **Zone Graph / Interiors** | `ZoneRegistry`/`ZoneTopology` + `generate_zones()`; `PortalPlugin` with `PlayerZoneTransition`; `ZoneNavGrids`; `UndergroundSimSchedule` (0.1 Hz) + `UndergroundPressure` + raid spawn; client `ZoneRendererPlugin` + `ZoneCache`; `underground_e2e` ralph scenario green | ✅ Scaffold complete — see follow-ups in `docs/systems/zones.md` (portal anchor world-coords, `ZoneTopology::shortest_path`, `ZoneTileMessage::kind`, lightyear per-zone interest groups) |
| **4 — MVF** | 3 character classes with distinct abilities; dungeon boss phases; faction consequences visible in-game (NPC alert patrol); party HUD health bars; ralph full suite green; 2-hour session stable | 🚧 Classes + abilities ✅; boss phases ✅; faction alert system ✅; party HUD ✅; roof cutaway ✅; zone-aware A* ✅. Remaining: dungeon room transitions (surface CaveEntrance portals + populated tile layouts), ralph full suite, 2-hour load test |
