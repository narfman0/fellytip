...
Grid dimensions are runtime-configurable via `MapGenConfig` (seed, width, height). Default values (`MAP_WIDTH = 1024`, `MAP_HEIGHT = 1024`) are constants in `crates/world-types/src/map.rs`; custom values are passed with `--seed`, `--map-width`, `--map-height` CLI args on the server. The `WorldMap` struct stores `width` and `height` fields so all indexing is self-contained. Cache files are named `world_{seed}_{width}x{height}.bin`.
...
All variants are listed in the `TileKind` enum in `crates/world-types/src/map.rs`.
