...
/// Three levels of detail, selected by distance from the camera to chunk centre.
///
/// | Level   | Step | Verts/side | Distance threshold |
/// |---------|------|------------|--------------------|
/// | Full    | 1    | 33         | < 80 world units   |
/// | Half    | 2    | 17         | 80–192 world units |
/// | Quarter | 4    | 9          | ≥ 192 world units  |
...
impl LodLevel {
    ...
    pub fn from_distance(dist: f32) -> Self {
        ...
        else                 { LodLevel::Quarter }
    }

    /// Next coarser level (saturates at Quarter).
    pub fn coarser(self) -> Self {
        match self {
            LodLevel::Full    => LodLevel::Half,
            LodLevel::Half    => LodLevel::Quarter,
            LodLevel::Quarter => LodLevel::Quarter,
        }
    }
}
